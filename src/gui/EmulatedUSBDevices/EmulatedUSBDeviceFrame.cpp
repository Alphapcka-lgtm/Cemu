...
wxPanel *EmulatedUSBDeviceFrame::AddSkylanderPage(wxNotebook* notebook) {
  ...
  row->Add(m_emulate_portal, 0, wxALIGN_CENTER_VERTICAL | wxALL, 5);
...
wxFlexGridSizer *EmulatedUSBDeviceFrame::AddSkylanderRow(uint8 row_number,
                                                         wxStaticBox *box) {

  auto *row = new wxFlexGridSizer(0, 5, 0, 0);
  row->SetFlexibleDirection(wxBOTH);
  row->SetNonFlexibleGrowMode(wxFLEX_GROWMODE_SPECIFIED);
  ...
  row->Add(m_skylander_slots[row_number], 0, wxALL | wxEXPAND, 5);
  ...
  row->Add(load_button, 0, wxALIGN_CENTER_VERTICAL | wxALL, 5);
  row->Add(clear_button, 0, wxALIGN_CENTER_VERTICAL | wxALL, 5);
